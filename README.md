...
実写4枚の実測値（`whiteRef` の中身ごと）:

| 白基準の中身 | 明るさ比(紙/肌) | 青寄り度 (B−R)/max | 彩度 (max−min)/max | |
|---|---|---|---|---|
| 白い紙 (8/30) | 1.24 | 0.000 | 0.014 | 正しい |
| 白い紙 (8/31) | 1.37 | −0.042 | 0.069 | 正しい |
| 顎の影の白いTシャツ | 1.31 | **+0.099** | 0.111 | 誤爆 |
| 首の肌 | **0.89** | −0.319 | 0.319 | 誤爆 |

- **★1つの指標で分けようとしないこと。**明るさ比だけではTシャツ(1.31)が正しい紙(1.24/1.37)の
  **間に入って**しまう。彩度だけでは正しい紙(0.069)とTシャツ(0.111)の差が薄く、しかも
  **ライブ判定(120px)と本判定(600px)でリサンプリング誤差だけで 0.099 / 0.111 と 0.10 をまたぐ**
  ＝1枚に張り付いた閾値になる。青寄り度なら 0.000 / −0.042 と +0.099 で、どちらにも 0.05 の余裕がある。
- 採用した3条件（`PH_WHITE_MIN_LUMA_RATIO` 1.05 / `PH_WHITE_BLUE_MAX` 0.05 / `PH_WHITE_SAT_MAX` 0.15）。
  明るさ比が首の肌を、青寄り度がTシャツを捕まえる。**どちらか片方では足りない。**
- **ライブ判定と本判定の両方に同じ3条件を入れてある。**片方だけ直すと「✓検出と出たのに撮ると却下」に戻る。
  ライブ側は `PH_WHITE_SAT_SLACK` のヒステリシス付き。
- 却下メッセージは「白い紙が見つかりません」。**首の肌のケースを色かぶりゲートより先に見る**のは、
  肌は彩度も高い(0.319)ため後回しにすると「照明の色が強く偏っています＝室内照明を消せ」という
  見当違いの指示になるため。
- 回帰ゲート: `40_photo_rejected_white_ref.png`（Tシャツ／首の肌の2枚を却下）と
  `41_photo_live_shirt_as_paper.png`（ライブで ✓検出 と出さない）。合成フレームの色は上表の実測値そのもの。
  **3条件を外すと本当に落ちることを確認済み**（外すと実写サンプルが「白い紙✓検出／顔の位置○ズレ」に戻る）。
...
| アプリ | 参照タグ | jsDelivr |
|---|---|---|
| colorlab | `@v1.17.0` | `https://cdn.jsdelivr.net/gh/thecompany20220901-cpu/colorlab-embed@v1.17.0/dist/colorlab.iife.js` |
