...
- **★このツールが出す a* は、実機より 3〜5 低い。**上の「色の未較正」と同じ理由で、
  RGB→I420(4:2:0)→RGB の往復で赤みが落ちるため。白紙によるWB補正を通したあとでも残る
  （2026-09-03 に `test/_probe_calib.mjs` で再測。理論 a*23.8/28.1/32.2/35.8/38.4/41.0 に対して
  届いたのは 20.0/23.7/27.1/30.9/34.4/38.0 ＝ **−3.0〜−5.1、平均 −4.2**）。
  **上限の議論に使うときは必ずこの補正を足すこと。**生値のまま上限を決めると、上限を低く付けすぎて
  実機で「肌以外を測ってしまいました」の誤却下が増える。
