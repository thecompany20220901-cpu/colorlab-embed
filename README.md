...
## 12タイプ結果画面の充実化（v1.20.7〜）

勝ち色を30色並べても「なぜ似合うのか」と「まず何を買えばいいのか」に答えていなかったので、
結果画面に4つ足した（2026-09-06）。**新しい色判定ロジックも新しい色も作っていない。**
使ったのは既にあるものだけ:

| 使ったもの | 何のために |
|---|---|
| `COLOR_CHECK`（24色 × 4タイプの ◎○△✕） | 顔映りチェック表の判定。「この色、似合う？チェッカー」で稼働中の自社テーブル |
| `hexToLab` / `deltaE` | 顔写真診断・コーデ採点と同じ Lab 変換と ΔE 照合 |
| `TYPES[].palette10` | タイプの基準色。中央値を基準値にする |
| `COLOR_FAMILIES` / `FAMILY_ORDER` | ベストカラーTOP6 の抽出と色相ファミリー |
| 商品マスタ `item_{site}.csv` の `color` 列 | おすすめ商品を TOP6 の色で絞り込む |

他社資料（MARROW / 16タイプカラーメソッド(R) / ICPA / clotteal 等）の数値・パレット・図版・
分類体系は**一切使っていない**。借りたのは「何をどの順で見せるか」という情報設計だけ。

### ① 色別 顔映りチェック表

`COLOR_CHECK` の24色を ◎→○→△→✕ の順に並べ、1色ずつ「顔まわりに置いたときの見え方」を
4軸で出す。判定は `COLOR_CHECK` のまま、コメントだけを Lab から機械的に作る。

| 軸 | 入力 | 段階 |
|---|---|---|
| 明るさ | `L*` − 基準L\* | 下がる（<-18）／やや下がる（<-6）／変わらない／やや上がる（<+18）／上がる |
| くすみ | `C*` − 基準C\* ＋ ベース（`b*`）のズレ | 出る／やや出る／出ない |
| 影 | 基準との明度差のうち**暗い方向だけ** | されない（<8）／弱い（<18）／少し出る（<30）／強調 |
| 血色 | 勝ち色への **ΔE**（色み `a*b*` を主・明度は 1/2 の重み）＋ `a*` で±1段 | 良い／普通／やや落ちる／悪い |

実装で踏んだ落とし穴が3つある。**同じ直し方が要る場所なので消さないこと。**

1. **基準は palette10 の平均ではなく中央値。** 平均だとイエベ春のアイボリー（L\*96）に
   引っ張られて基準が L\*77 になり、ほぼ全色が「明るさ:下がる」になる。
2. **影は暗い方向だけ見る。** 明度差の絶対値で取ると、基準が L\*44 のブルベ冬で
   明るい色まで「影:強調」になる。影は暗い色が作るもの。
3. **白（L\*≥78）と黒（L\*≤25）はくすみ判定から外す。** 彩度だけで見ると白（C\*=0）が
   全タイプ「くすみ:出る」になり、クリアな無彩色を濁った色と誤判定する。

**判定を ΔE から計算し直してはいけない。** `COLOR_CHECK` と食い違うと、同じ色が
結果画面では △、チェッカー画面では ◎ という状態になる。

暗い色は、似合う色（◎）でも「明るさ:下がる／影:強調」と出る。これは正しい挙動なので
直さないこと（ブルベ冬のブラック・ネイビーがそう出る）。表の末尾にその旨の注記を置いてある。

### ② ベストカラーTOP6

勝ち色30色のうち上位6色を、結果画面のいちばん上に帯で出す。抽出は2段:

1. `COLOR_FAMILIES` の `✓`（＝`COLOR_CHECK` で ◎ の最優先色）を ΔE が近い順に
2. 足りない分は、**まだ使っていない色相ファミリーを優先して** ΔE 順

2 のファミリー優先は実測して足した。ブルベ夏は `✓` が4色しかなく、ΔE順だけで埋めると
グレー・オフホワイト・ソフトグレーの3兄弟が並んで「ベストカラー」に見えない。

実測結果（2026-09-06）:

| タイプ | TOP6 |
|---|---|
| イエベ春 | イエロー・コーラルピンク・サーモンピンク・キャメル・ベージュ・ミントグリーン |
| イエベ秋 | ブラウン・マスタード・テラコッタ・ベージュ・キャメル・カーキ |
| ブルベ夏 | 青みピンク・水色・ラベンダー・グレー・ミントグリーン・ベビーピンク |
| ブルベ冬 | ビビッドピンク・エメラルド・ロイヤルブルー・パープル・ネイビー・ホワイト |

### ③ おすすめ商品を TOP6 の色で絞り込む

**`SKUS[]` は色の情報を持っていない**（`id/name/price/cat/tpo/frame` だけ。商品名にも
色語は 0件/25件）。そこで商品マスタ `item_{site}.csv` の `color` 列を
`test/build_sku_colors.py` で抜き、`src/sku_color_data.js`（`SKU_COLORS`）に持たせている。

**商品写真から服の色を測ってはいけない。** 一度やって失敗している。モデル写真は背景・肌・
羽織り・影が混ざって18件中5件を外したうえ、根本的に**これらの商品は1点1色ではなく
色展開を複数持つ**（`2129` はホワイト/グリーン/ピンク/ボルドー/グレーの5色展開）。
写真から測れるのは「撮影に使った1色」だけで、商品の色展開とは別物。

マスタの色語（19種）とアプリの色名は `MASTER_COLOR_ALIAS` で突き合わせる。**HEXは作らない。**
表の右側はすべて `palette10` / `COLOR_FAMILIES` に実在する名前だけ。
ゴールド・シルバーは金属で服の色ではないので、この表に入れない＝絞り込みの対象外。

絞り込みの実測（2026-09-06）: イエベ秋 9/13点・ブルベ夏 6/12点。
なお**マスタの `color` 列は誤記が出やすい**ので、絞り込み結果に違和感が出たらまずそこを疑う。

### ④ 避けたい色を着たいときは

苦手色ブロックの直後に、回避策3点（顔から離す／顔まわりに明るい色/メイクとアクセ）。

**見出しに「苦手な色」を使ってはいけない。** `test/screenshot.mjs` が
「苦手な色」の出現回数が1回であること（＝苦手色ブロックが2箇所に散らばっていないこと）を
検査しているため、2回になって落ちる。①の ✕ グループが「顔まわりでは避けたい色」なので
語を「避けたい色」に揃えてある。

**研究所／EC公式の出し分けは入れていない。** このアプリの設置先は blubel.jp / iebel.jp の
2つでどちらもEC公式であり、研究所（blube_lab / iebe_lab）はIGアカウント側でこのアプリの
設置先ではない。「記事への埋め込み」の節にあるとおりホスト名は一切見ない設計で、
既存の苦手色ブロックも両サイトに出ている。出し分けが要るなら `#colorlab-root` の
data属性で制御すること（ホスト名判定は採らない）。

### ⑤ アクセサリーの金属

ゴールド / シルバーは **Q10（金属の設問イラスト）の色をそのまま流用**し、勝ち色への ΔE で
判定する（≤12=◎ / ≤22=○ / ≤40=△ / それ以上=✕）。実測:

| | イエベ春 | イエベ秋 | ブルベ夏 | ブルベ冬 |
|---|---|---|---|---|
| ゴールド `#D4AF5A` | ◎ 11.5 | ◎ 9.8 | ✕ 52.8 | ✕ 53.8 |
| シルバー `#B8BEC9` | △ 25.2 | △ 25.2 | ◎ 1.8 | ◎ 5.7 |

パールは**どのタイプにも似合う**ので ◎ 固定にし、差が出る「白みの選び方」の方を出す
（春=アイボリー / 夏=オフホワイト / 秋=ベージュパール / 冬=ピュアホワイト。いずれも
そのタイプの `palette10` に実在する色）。パール用の HEX は自社に無かったので作っていない。

### 検証

`test/facecheck_check.mjs`（4タイプ × 90項目）。実ブラウザで12タイプ診断を最後まで通し、
回答パターンを変えて4タイプすべてに到達させる。**24色 × 4軸 = 96個のコメントと、TOP6、
絞り込み件数を、テスト側で独立に再計算して突き合わせる**（表示ロジックをそのまま呼ばない）。
検収用のスクショは `test/_facecheck_shots.mjs` で `test/screenshots/enrich_*.png` に出る。

## デパコスAFF（A8.net 経由・Qoo10 / v1.20.6〜）

12タイプ結果の「おすすめコスメ」一覧が KATE / CANMAKE / CEZANNE / rom&nd などのプチプラ
だけだったため、各タイプに**デパコスのリップを1点ずつ**足した（2026-09-05）。服が大人っぽいのに
コスメがプチプラだらけ、という指摘への対応。

| タイプ | 商品 | Qoo10 | a8adId |
|---|---|---|---|
| イエベ春 | Dior アディクト リップグロウバター 106 Pomelo | `/g/1166428287` | `4An06KR-g7-vkg5yTm` |
| イエベ秋 | SUQQU シアーマット リップスティック 112 芳実（レフィル） | `/g/1117415069` | `4An06KR-g7-vkg5h7u` |
| ブルベ夏 | SUQQU シアーマット リップスティック 111 熟葡萄（レフィル） | `/g/1117415069` | `4An06KR-g7-vkg5h7u` |
| ブルベ冬 | SUQQU シアーマット リップスティック 111 熟葡萄（レフィル） | `/g/1117415069` | `4An06KR-g7-vkg5h7u` |

- データは `COSME_FULL[type]` の**末尾**に `isDepacosme: true` で置く。**先頭に置かないこと**。
  リップ試着画面が `COSME_FULL[myType].filter(x => x.cat === "リップ").slice(0, 3)` で
  先頭3件を拾っており、先頭に入れると既存の3点が押し出される。
- `price: null`。Qoo10 は価格が動くので載せない。`CosmeCard` は `typeof item.price === "number"`
  で判定して価格欄ごと省く（**ガードを外すと `price.toLocaleString()` で落ちる**）。
- `set` は付けない。「一式セット」の合計 `filter(x => x.set).reduce(...)` に混ざらないようにするため。
- 表示は `DepacosCard`。淡い地色 + 黒の `DEPACOS` ラベル + 商品写真で、文字だけのプチプラカードと
  一目で区別できるようにしている。

### A8 の商品リンクの出し方（ad.js を動的ロードする）

A8 が発行するタグは「`<span class="a8ad {id}">` を置いて `ad.js` に innerHTML させる」方式だが、
**そのまま JSX に貼っても動かない**。理由は2つ:

1. JSX に書いた `<script>` は実行されない。
2. そもそも設置先の blubel.jp / iebel.jp は Next.js の SPA で、本文の `<script>` は
   ブラウザ仕様上実行されない（「記事への埋め込み」の節と同じ問題）。

そこで `loadA8()` がカードの mount 時に `https://statics.a8.net/ad/ad.js` を**1回だけ**動的ロードし、
`a8adscript("body").showAd({...})` を呼ぶ。`ad.js` の実物を読んで確認した注意点:

- **`goods.imu` は「商品名」ではなく画像URL**。`chkUrl`（`^https?://`）を通らないと `showAd()` が
  `false` を返して**何も描画されない**。A8 の管理画面のサンプルを商品名で埋めると無言で死ぬ。
  使っている画像は Qoo10 の商品画像（`gd.image-qoo10.jp/li/.../*.g_400-w_g.jpg`・400x400）。
- 差し込み先の探し方が `$("." + req.id)` の**最後の1件**。該当 span が DOM に無い状態で
  `showAd()` を呼ぶと TypeError になるので、`ref` が生きているかを見てから呼ぶ。
- 差し込むのは `<a href="//px.a8.net/svt/ejp?a8mat=...&a8ejpredirect=...">` + `<img>` と、
  `www17.a8.net/0.gif` の 1x1 計測gif。**span は JSX で子要素を持たせない**こと
  （React は自分が作っていない子ノードは消さないので、この形なら共存できる）。
- 画像の大きさは `base.colorlab.css` の `#colorlab-root .a8ad-slot a img` で枠幅に合わせている。
  `a img` に限定しているのは、直下の 1x1 計測gif まで引き伸ばさないため。

### フォールバックと計測URL

`ad.js` が落ちた・広告ブロックされた場合は、同じ遷移先の直リンクに切り替える。

```
https://px.a8.net/svt/ejp?a8mat=4B3VR9+GAG5TE+4QYG+BWGDT&a8ejpredirect={Qoo10のURLエンコード}
```

これは `ad.js` が画像に付けるのと同一のURLなので、フォールバックしても成果は落ちない。
「Qoo10で見る」ボタンは**正常時・フォールバック時とも常時表示**する（商品画像だけだと
リンクだと気づかれないため）。

検証は `test/depacosme_check.mjs`。4タイプ × ad.js正常時／`statics.a8.net` を abort した
フォールバック時／ボタンの実クリック（`px.a8.net` は abort してURLだけ実測。A8 に偽のクリックを
記録させない）を通す。
...
- **v1.22.2 は両サイトとも本番切替済み・実測確認済み**（2026-09-19。Keisuke が GTM で `@v1.22.0` → `@v1.22.2`）。
  ...
  **注意: v1.22.1 以前のアプリは非公開の応答で画面が壊れる**ため、本番の `KOTAE_STATS_PUBLIC` は GTM が `@v1.22.2` に
  切り替わったのを確かめてから "0" にして deploy する（それまで "1"）。LP は `lp/build_kotaeawase_lp.py` で BLUBEL / IEBEL 別に生成
  （景品候補は `lp/kotaeawase_prizes.json`）。バンドル 891,221B。検証: verify 168/0・kotaeawase 67/67・mine_account 22/22・mine_worker 90/90。
  Worker `dc9d550a-1bc6-4bbc-9ead-e498c49a0496`（1つ前 `6d00bdcf-4fb9-4a1f-9743-4e70cff9bf3a`）。
- **v1.22.1 はタグ発行済み・本番未切替のまま v1.22.2 に置き換え**（2026-09-19）。GTM のタグを `@v1.22.0` → `@v1.22.1` に書き換えると反映
  （IEBEL＝`GTM-WVFLHTNW` ／ BLUBEL＝`GTM-NN7QDLCH`・Keisuke作業）。ロールバックは `@v1.22.0` に戻すだけ。
  中身: 答え合わせキャンペーンの確定内容を反映（keisuke 2026-09-19）。ハッシュタグ `#答え合わせキャンペーン #ColorLabMINE`・
  メンション先はアプリの結果（写真で診断の 1st）でブルベ→`@blube_lab` / イエベ→`@iebe_lab`（結果画面とストーリー画像）・
  当選発表の文言・実施期間の表示。**実施期間はアプリに書かず Worker の `KOTAE_START` / `KOTAE_END`（JST・両端含む）から出す**
  （全ての実装・テスト完了の翌日から1週間。日付が決まったら vars を入れて `wrangler deploy` するだけ・再リリース不要）。
  期間外の回答は Worker が記録しない（開始前 403 `not_started` / 終了後 410 `ended`）、画面は「集計に入りません」と期間を出す。
  バンドル 890,751B（gzip 397KB）。検証: verify 168 OK/0 NG・kotaeawase 45/45・mine_account 22/22・mine_worker 84/84。
  Worker は先にデプロイ済み `6d00bdcf-4fb9-4a1f-9743-4e70cff9bf3a`（v1.22.0 のアプリでもそのまま動く）。1つ前は `57d939a6-de73-4a9a-935d-5430a18e44df`。
...
- **v1.20.7 は両サイトとも本番切替済み・実測確認済み**（2026-09-06）。12タイプ結果画面に
  ...
  「12タイプ結果画面の充実化」の節。v1.20.6 のデパコスAFF も内包しているため、
  本番は `@v1.20.5` から v1.20.6 を飛ばして `@v1.20.7` へ1手で上げた。
  - 実測（`test/_v1207_live.mjs`・本番の実ブラウザ通し・**2サイト計61項目すべてPASS**）:
    読み込まれるバンドルが両サイトとも `@v1.20.7` ／ 顔映りチェック表の
    **24色 × 4軸 = 96個のコメントが Lab から独立に再計算した期待値と不一致0件**（両サイト）／
    判定の並びが `COLOR_CHECK` と完全一致 ／ TOP6 の絞り込みが BLUBEL 6/12点・IEBEL 9/13点で
    実際にクリックして効く ／ 金属が ΔE どおり（冬=シルバー◎5.7・ゴールド✕53.8、
    春=ゴールド◎11.5・シルバー△25.2）。
  - 回帰なし: 勝ち色・苦手色（1箇所のまま）・1st/2nd・コスメ・おすすめ商品・専門表記・
    研究所監修表記・デパコスAFF（A8計測URLと「Qoo10で見る」まで実物で確認）。
  - **JSエラー0件**（両サイト）。requestfailed はすべて `ERR_ABORTED`
    （BLUBEL 31件・IEBEL 24件＝GA等のビーコンとSPA遷移の中断）で、中断以外の失敗は0件。
  - jsDelivr 実測: 844,271 bytes・ローカル `dist` と SHA-256 一致。
- **v1.20.6 はタグ発行済み。ただし本番へは出さないまま v1.20.7 に内包された**（2026-09-05）:
  **12タイプのおすすめコスメにデパコスを1点ずつ追加**（A8.net 経由・Qoo10）。
  詳細は「デパコスAFF」の節。2026-09-06 の実測で本番は `@v1.20.5` のままだったため、
  切替は v1.20.6 を飛ばして v1.20.7 に進める。
...
- **v1.20.5 は両サイトとも本番切替済み・実測確認済み**（2026-09-05）。
  `test/_v1205_live.mjs`（本番の実ブラウザ通し・34項目×2サイト）と
  `test/_v1205_live_credit.mjs`（監修表記の出し分けと requestfailed の内訳）で確認した。
  実測: 読み込まれるバンドルが `@v1.20.5` / おすすめ商品6点が fulmo-img-server.com から
  全点表示・リンクは閲覧中サイトのみ / 保存PNGは 1080x1920 のままで監修表記は下部帯と
  すき間51px / 既存導線（写真・質問・写真+質問・シーズンアバター・1st2nd）に回帰なし /
  JSエラー0件。requestfailed はすべて `net::ERR_ABORTED`（GA等のビーコンとSPAの中断）で、
  中断以外の失敗は0件。
  なお**監修表記は閲覧中サイトではなく1位タイプの担当サイトで決まる**。
  blubel.jp でイエベ春が出れば「イエベ研究所監修 @iebe_lab」が正しい。
...
- **v1.20.5 の内容（タグ未発行）**: **「自分の顔で作る」の結果画面に、おすすめ商品を追加**（2026-09-05）。
  ...
  データ源は既存のものだけで、**新しいマッピングは作っていない**:
  タイプ→サイトは `TYPES[].site`、サイト→在庫商品は `SKUS[site]`（「在庫あり・直近2ヶ月売れ筋」の実データ。
  在庫切れはそもそもこの配列に入らない）、商品画像は `SKU_IMG[site][id]`（商品マスタ `images1`・25SKU全件）、
  リンクは `ITEM_URL(site, id)`（UTM付き）。並べ方も新規ではなく、「ふたりの相性配色」の
  Your Pair Coordinate と同じ**トップス → ワンピース/ボトムス/セットアップ → 小物**のカテゴリ散らしを再利用し、
  横スクロールで最大6点（`CARD_SKU_LIMIT`）出す。
  **出す商品は「閲覧中のサイト」のものだけ**で、他サイトの商品は混ぜない。閲覧中サイトは
  `embedSite()` がホスト名（`*.blubel.jp` / `*.iebel.jp`）から判定する。判定できない環境
  （ローカル検証など）ではタイプ既定の `TYPES[].site` にフォールバックする。
  閲覧中サイトがそのタイプの担当サイトと違うとき（例: iebel.jp でブルベ夏）は、
  「このタイプに似合う」と言い切らない文言に切り替える。
  GA4 は `card_sku_click` を追加。アバタールートは元から商品連携が無く、今回も出していない
  （`CARD_SKU_ON_AVATAR = true` にすれば同じ欄が両方に出る）。
  **検証は `node test/card_sku_check.mjs` の 42 項目**（BLUBEL/IEBEL 両方で、件数・画像・商品名・価格・
  リンク先・在庫データとの突き合わせ・他サイト混入なし・保存PNGが 1080x1920 のまま・
  アバタールートは従来どおり商品欄なし）。
  バンドルは 822,749 → 826,420 bytes（+3,671 / +0.4%）。
- **v1.20.0 の内容（タグ未発行・keisuke確認待ち）**: **パーソナルカラーカード**を追加。
  ...
  **入口B**＝ホーム最上段の「あなたの個性色が分かる！」から。入口Bは端末に診断結果があれば
  入口Aと同じ2問に合流し、無いときだけ色の2択6ペア（Q3）を足して 1位/2位 を推定する。
  Q3の採点は**本番Q12と同じ加点式・重み一律+1・同点はタイプ番号昇順**（`sortTypes` をそのまま再利用）で、
  入口Bと本診断で判定基準がズレない。文言は 12タイプ × Q1 × Q2 = **48通りを `src/card_data.js` に固定**し、
  ランタイムでAIを呼ばない。色は `TYPES[].palette10` からの引用のみで新規の色は作っていない。
  アバターは **シーズン4 × 個性4 = 16種**（`dist/avatars/*.webp` 512x768）で、常時読み込みの
  `colorlab.iife.js` を太らせないため**バンドルに内包せず jsDelivr から遅延読み込み**する。
  配信元は**動作中の `colorlab.iife.js` 自身のURLから導出**するので、タグを上げても画像URLの直しは不要。
  GA4 は `personalcolor_card_generated` / `card_to_diagnosis_click` / `card_saved` を `dataLayer` に push し、
  送客リンクとカード内URLに `?ref=colorcard` を付ける。フッター帯の色だけサイト別
  （BLUBEL `#F5F2FA` / IEBEL `#FAF4EE`）。**検証は `node test/card_smoke.mjs` の 35 項目。**
  バンドルは 786,958 → 815,595 bytes（+28,637 / +3.6%）。
