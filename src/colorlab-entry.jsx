...
// 答え合わせキャンペーン（MINE v1）。入口は /pages/personalcolor?campaign=kotaeawase
// （2026-09-19 keisuke 決定: GTM は変えない。GTM が読み込むのは personalcolor だけ）。
// 本文の <div id="colorlab-root" data-mode="kotaeawase"> でも有効になる。
...
// MINE 会員画面。?mine=account か、ログインメールのリンク（?mine_token=...）で開く
function accountOf() {
  try {
    const q = new URLSearchParams(location.search);
    return q.get("mine") === "account" || !!q.get("mine_token");
...
function mount(target) {
  ...
  const campaign = campaignOf(el);
  root.render(React.createElement(App, { campaign, account: !campaign && accountOf() }));
