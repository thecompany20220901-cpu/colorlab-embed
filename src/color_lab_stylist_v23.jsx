...
/* 白基準が「十分明るい」「肌より明るい」「色が濃くない」ことを求める。
   ★1つの指標で分けようとしないこと。実測（正しい紙4枚 + 誤爆2枚）:
       白基準の中身              明るさwMax  明るさ比  青寄り度  彩度
       ...
       白いTシャツ(顎の影)          171      1.31    +0.099    0.111   ← 誤爆
       首の肌                       135      0.89    -0.326    0.326   ← 誤爆
   ★2026-09-03 は正しい紙が2枚（青寄り度 0.000 / -0.042）しか無く、青寄り度 0.05 で
     切れると判断した。9/4 に正しい紙をあと2枚もらったところ **+0.079 / +0.066** で、
     正しい紙のほうが誤爆(+0.099)に肉薄していた。青寄り度でも彩度でも、正しい紙と誤爆の
     差はもう 0.02 しかなく、**どちらも分離には使えない**。
     一方 **白基準の明るさ(wMax) は 215〜228 と 135〜171 で 44 も離れている**。
     窓の光を拾って持つ紙は明るく写り、体に貼り付いた影の中の服は暗い、という
     物理的な差なので、ここを主の分離軸にする。
   ★青寄り度・彩度は「極端に色の付いた面」を止めるだけの保険に格下げし、
     正しい紙の実測最大(0.079)から十分離した値にする。分離はさせない。 */
const PH_WHITE_MIN_LEVEL = 190;  // 白基準の最大チャンネル。これ未満＝紙が無い/暗すぎて基準にならない
const PH_WHITE_SAT_MAX = 0.15;   // 保険: 色が濃い面（正しい紙の実測は 0.079 まで）
const PH_WHITE_BLUE_MAX = 0.12;  // 保険: 極端に青い面（正しい紙の実測は +0.079 まで）
...
function livePhotoCheck(video, canvas) {
  ...
  // 本判定の白基準ゲートとそろえる。ここを従来の「色かぶり0.28だけ」にしておくと、
  // 白いTシャツを ✓検出 と出したまま歪んだモノサシで肌を測る（2026-09-02 実機）。
  const paper = bright && wMax >= PH_WHITE_MIN_LEVEL && wBright
    && wBlue <= PH_WHITE_BLUE_MAX && wSat <= PH_WHITE_SAT_MAX;
  ...
  //   paper だけ余裕つきで✓に貼り付くと「白い紙 ✓検出／顔の位置 —判定待ち」という
  //   自己矛盾した表示になる（2026-09-04 実機で発生）。paper は多数決だけで平滑化する。
...
async function aiPhotoDiagnose(base64, mediaType) {
  ...
  // 白基準が暗い = 紙を持っていないか、紙が影に沈んでモノサシとして使えない。
  // 正しい紙の実測は 215〜228、誤爆した服・肌は 135〜171（2026-09-04 時点・6枚）。
  if (wMax < PH_WHITE_MIN_LEVEL) {
    throw gateError("白い紙が見つかりません",
      "顎の下に白い紙かハンカチを持ち、窓の近くで白い紙が明るく写るようにして撮り直してください。白い服や肌は白の代わりになりません。");
  }
