        ...
        {mode === "home" && (
          <div className="fade-up">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { if (!AI_ENABLED) { setSoonOpen(true); return; } setStResult(null); setMode("stylist"); }} className="w-full flex items-center gap-4 rounded-2xl p-5 text-left transition-shadow hover:shadow-md" style={{ border: AI_ENABLED ? `2px solid ${C.main}` : "2px solid #dedede", background: AI_ENABLED ? "#fdfbfd" : "#f7f7f7" }}>
                <span className="w-11 h-11 rounded-full flex items-center justify-center shrink-0 text-white" style={{ background: AI_ENABLED ? C.main : "#bdbdbd" }}><Shirt size={20} /></span>
                <span className="min-w-0 flex-1"><span className="block text-sm font-medium" style={{ color: AI_ENABLED ? C.ink : "#9a9a9a" }}>パーソナルカラー別コーデ提案{!AI_ENABLED && <span className="ml-2 align-middle text-[10px] px-2 py-0.5 rounded-full" style={{ background: "#e4e4e4", color: "#8a8a8a" }}>近日公開</span>}</span><span className="block text-xs mt-0.5" style={{ color: AI_ENABLED ? C.sub : "#ababab" }}>シーン×気分から、服とメイクを提案</span></span>
                <ArrowRight size={16} style={{ color: AI_ENABLED ? C.main : "#bdbdbd" }} />
              </button>
            </div>
