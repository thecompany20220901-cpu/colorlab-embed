...
function buildShareImage(RT, secondName, axes) {
  ...
  // フッター（カタログ領域の下・カード内に収まる位置）
  ctx.textAlign = "center";
  ctx.fillStyle = "#3a3340"; ctx.font = "600 42px sans-serif";
  ctx.fillText("あなたも診断してみて ♡", W / 2, cardY + 1520);
  ctx.fillStyle = "#a99fa8"; ctx.font = "36px sans-serif";
  ctx.fillText(RT.sns + "  |  " + RT.siteName, W / 2, cardY + 1585);
...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full flex items-start justify-center p-4 font-sans" style={{ background: "linear-gradient(160deg,#fbf9f7 0%,#f3eef2 50%,#eef2f4 100%)" }}>
      ...
      `}</style>
      {/* 「近日公開」モーダル。ホーム以外(診断結果・骨格結果)のCTAからも開くため最上位に置く。 */}
      {soonOpen && (
        ...
        {mode === "home" && (
          <div className="fade-up">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { if (!AI_ENABLED) { setSoonOpen(true); return; } setStResult(null); setMode("stylist"); }} className="w-full flex items-center gap-4 rounded-2xl p-5 text-left transition-shadow hover:shadow-md" style={{ border: `2px solid ${C.main}`, background: "#fdfbfd" }}>
                ...
                <span className="min-w-0 flex-1"><span className="block text-sm font-medium" style={{ color: C.ink }}>パーソナルカラー別コーデ提案{!AI_ENABLED && <span className="ml-2 align-middle text-[10px] px-2 py-0.5 rounded-full" style={{ background: "#ececec", color: "#8a8a8a" }}>近日公開</span>}</span><span className="block text-xs mt-0.5" style={{ color: C.sub }}>シーン×気分から、服とメイクを提案</span></span>
                <ArrowRight size={16} style={{ color: C.main }} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {[
                ...
                <button key={i} onClick={it.onClick} className="flex items-center gap-2.5 rounded-2xl px-3.5 py-4 text-left transition-shadow hover:shadow-md" style={{ border: "1px solid " + C.line }}>
                  ...
                  <span className="text-xs font-medium leading-tight" style={{ color: C.ink }}>{it.label}{it.soon && <span className="block mt-0.5 text-[9px] px-1.5 py-0.5 rounded-full w-fit" style={{ background: "#ececec", color: "#8a8a8a" }}>近日公開</span>}</span>
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {[
                ...
                { icon: <Check size={17} />, label: "今日のコーデ採点", soon: !AI_ENABLED, onClick: () => { if (!AI_ENABLED) { setSoonOpen(true); return; } setScPreview(null); setScResult(null); setScError(""); setMode("score"); } },
              ].map((it, i) => (
                <button key={i} onClick={it.onClick} className="flex items-center gap-2.5 rounded-2xl px-3.5 py-4 text-left transition-shadow hover:shadow-md" style={{ border: "1px solid " + C.line }}>
                  <span className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: "#f4eff3", color: C.main }}>{it.icon}</span>
                  <span className="text-xs font-medium leading-tight" style={{ color: C.ink }}>{it.label}{it.soon && <span className="block mt-0.5 text-[9px] px-1.5 py-0.5 rounded-full w-fit" style={{ background: "#ececec", color: "#8a8a8a" }}>近日公開</span>}</span>
        ...
        {mode === "quiz" && quizResult && RT && (
          <div className="fade-up">
            ...
            <div className="px-8 py-9">
              ...
              {COSME[RT.key].slice(0, 3).map((item, i) => <CosmeCard key={i} item={item} />)}
              ...
              <button onClick={() => { if (!AI_ENABLED) { setSoonOpen(true); return; } setStResult(null); setMode("stylist"); }} className="block w-full text-center px-6 py-3.5 rounded-full text-white text-sm font-medium mb-3 mt-2 transition-transform hover:scale-105" style={{ background: RT.accent }}>このタイプで「今日なに着る？」→</button>
                ...
                <>
                  ...
                  {cosmeCat === "一式セット" ? (
                    <>
                      ...
                      {COSME_FULL[myType].filter((x) => x.set).map((item, i) => (
                        <div key={i}>
                          <CosmeCard item={item} />
                          <div className="flex items-center gap-1.5 -mt-1 mb-3 pl-1">
                            <span className="text-[10px]" style={{ color: C.faint }}>似合うカラー:</span>
                            {TYPES[myType].palette.slice(0, 5).map((c, j) => <span key={j} className="w-4 h-4 rounded-full" style={{ background: c, border: "1px solid #e8e2e8" }} />)}
                          </div>
                        </div>
                      ))}
                    </>
                  ) : (
                    (cosmeCat === "すべて" ? COSME_FULL[myType] : COSME_FULL[myType].filter((x) => x.cat === cosmeCat)).map((item, i) => (
                      <div key={i}>
                        <CosmeCard item={item} />
                        <div className="flex items-center gap-1.5 -mt-1 mb-3 pl-1">
                          <span className="text-[10px]" style={{ color: C.faint }}>似合うカラー:</span>
        ...
        {mode === "frame" && frameDone && myFrame && (
          <div>
            ...
            <div className="px-8 py-9">
              ...
              <div className="grid grid-cols-2 gap-4 mb-7">
                <div><div className="text-xs mb-2" style={{ color: C.faint }}>得意なデザイン</div><div className="grid grid-cols-2 gap-2">{FRAMES[myFrame].good.map((g, i) => <div key={i} className="flex items-center gap-2 rounded-xl px-2.5 py-2" style={{ background: FRAMES[myFrame].accent + "10", color: FRAMES[myFrame].accent }}><DesignIcon label={g} /><span className="text-[11px] leading-tight font-medium">{g}</span></div>)}</div></div>
                <div><div className="text-xs mb-2" style={{ color: C.faint }}>注意したいデザイン</div><div className="grid grid-cols-2 gap-2">{FRAMES[myFrame].ng.map((n, i) => <div key={i} className="flex items-center gap-2 rounded-xl px-2.5 py-2" style={{ background: "#f2eef1", color: "#8a828d" }}><DesignIcon label={n} /><span className="text-[11px] leading-tight font-medium">{n}</span></div>)}</div></div>
              </div>
              {myType ? (
                <>
                  ...
                  <button onClick={() => { if (!AI_ENABLED) { setSoonOpen(true); return; } setStResult(null); setMode("stylist"); }} className="block w-full text-center px-6 py-3.5 rounded-full text-white text-sm font-medium mb-3 mt-2 transition-transform hover:scale-105" style={{ background: TYPES[myType].accent }}>カラー×骨格で「今日なに着る？」→</button>
                </>
              ) : (
                <>
          ...
          <div>
            ...
            <div className="px-8 pb-12 pt-3">
              ...
              {myType && (
                <>
                  <div className="inline-flex items-center gap-2.5 mb-3 px-6 py-3 rounded-full text-2xl font-bold" style={{ background: TYPES[myType].accent + "14", color: TYPES[myType].accent }}>
                    {TYPES[myType].name} <button onClick={() => { setMyType(null); setScResult(null); }} className="underline text-xs font-normal" style={{ color: C.faint }}>変更</button>
                  </div>
                  <p className="text-xs leading-relaxed mb-4" style={{ color: C.sub }}>
